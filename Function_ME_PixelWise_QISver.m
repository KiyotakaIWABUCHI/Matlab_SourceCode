function [bitplane_MC,Estimation_x,Estimation_y]=Function_ME_PixelWise_QISver(bitplanes,range_x,range_y,K,n,downsample,BoxK,M)
%%%%%%%%%%%%%%%%%%%%%%%%%
...
%             dy=fix((t-1)*shift_per_bitplane_y);
       ...
       %% コスト計算
        [result_2D]=Function_Module_Chi2MapCul_Mpixel_Boxfilt(tmp_bitplane,downsample,1,M);  
        result_2D=imresize(result_2D,[TATE YOKO],'bicubic');
        result_2D=imboxfilt(result_2D,BoxK);
       %% 縮拡で近傍画素考慮
       if(y<0)
            if(x<0)
       ...
       %% ビットプレーン更新
        ...
        imshow(uint8(Function_Reconstruction_MLE_Oversample(bitplane_MC,1,1,5)))
     end
end
