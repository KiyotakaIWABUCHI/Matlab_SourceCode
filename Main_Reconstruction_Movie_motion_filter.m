...
for offset=-30:-30
    ...
    for set=2:2
        ...
        imgs_update=imgs;
        CHI_Maps_update=CHI_Maps;
        
        for t=0:Loop_Num-1
            ME_x=ME_results(1,t+1,2);
            ME_y=ME_results(2,t+1,2);
            Motion=sqrt(ME_x*ME_x+ME_y*ME_y);
            
            if(sqrt(ME_y*ME_y+ME_x*ME_x)>=(MF-5) && sqrt(ME_y*ME_y+ME_x*ME_x)<=(MF+5))
                for i=1:SIZE(1)
                    for j=1:SIZE(2)
                        chi_min=realmax;
                        if(CHI_Maps(i,j,t+1)<=K_th)
                            imgs_update(i,j,t+1)=imgs(i,j,t+1);
                            CHI_Maps_update(i,j,t+1)=CHI_Maps(i,j,t+1);
                        else
                            for t_tmp=0:Loop_Num-1
                                ME_x_before=ME_results(1,t_tmp+1,1);
                                ME_y_before=ME_results(2,t_tmp+1,1);
                                ME_x_after=ME_results(1,t_tmp+1,3);
                                ME_y_after=ME_results(2,t_tmp+1,3);
                                Motion_before=sqrt(ME_y_before*ME_y_before+ME_x_before*ME_x_before);
                                Motion_after=sqrt(ME_y_after*ME_y_after+ME_x_after*ME_x_after);
                                
                                if(CHI_Maps_before(i,j,(t_tmp+1))<chi_min && Motion_before>=(MF-5) && Motion_before<=(MF+5))
                                    imgs_update(i,j,t+1)=imgs_before(i,j,t_tmp+1);
                                    CHI_Maps_update(i,j,t+1)=CHI_Maps_before(i,j,t_tmp+1);
                                    chi_min=CHI_Maps_before(i,j,(t_tmp+1));
                                end
                                if(CHI_Maps_after(i,j,(t_tmp+1))<chi_min && Motion_after>=(MF-5) && Motion_after<=(MF+5))
                                    imgs_update(i,j,t+1)=imgs_after(i,j,t_tmp+1);
                                    CHI_Maps_update(i,j,t+1)=CHI_Maps_after(i,j,t_tmp+1);
                                    chi_min=CHI_Maps_after(i,j,(t_tmp+1));
                                end
                ...
                for i=1:SIZE(1)
                    for j=1:SIZE(2)
                        chi_min=realmax;
                        if(CHI_Maps(i,j,t+1)<=K_th_noninst)
                            imgs_update(i,j,t+1)=imgs(i,j,t+1);
                            CHI_Maps_update(i,j,t+1)=CHI_Maps(i,j,t+1);
                        else
                            %a="error"
                            for t_tmp=0:Loop_Num-1
                                ...
                                if(CHI_Maps_before(i,j,(t_tmp+1))<chi_min && Motion_before>=(Motion-5) && Motion_before<=(Motion+5))
                                    imgs_update(i,j,t+1)=imgs_before(i,j,t_tmp+1);
                                    CHI_Maps_update(i,j,t+1)=CHI_Maps_before(i,j,t_tmp+1);
                                    chi_min=CHI_Maps_before(i,j,(t_tmp+1));
                                end
                                if(CHI_Maps_after(i,j,(t_tmp+1))<chi_min && Motion_after>=(Motion-5) && Motion_after<=(Motion+5))
                                    imgs_update(i,j,t+1)=imgs_after(i,j,t_tmp+1);
                                    CHI_Maps_update(i,j,t+1)=CHI_Maps_after(i,j,t_tmp+1);
                                    chi_min=CHI_Maps_after(i,j,(t_tmp+1));
                                end
        ...
        [Chis_sort,rank_order_update]=sort(CHI_Maps_update,3);
        [Chis_sort,rank_order_tmp]=sort(CHI_Maps,3);
        
        
        %% Rank
        %         w_total_rank=zeros(SIZE);
        img_result_rank=zeros(SIZE);
        img_result_denose=zeros(SIZE);
        img_result_rank_tmp=zeros(SIZE);
        cnt_result_denose=ones(SIZE);
        %
        ...
        for i=1:SIZE(1)
            for j=1:SIZE(2)
                img_result_rank(i,j)=imgs_update(i,j,rank_order_update(i,j,1));
                img_result_rank_tmp(i,j)=imgs(i,j,rank_order_tmp(i,j,1));
            end
        end
        ...
        for i=1:SIZE(1)
            for j=1:SIZE(2)
                for t=0:Loop_Num-1
                    if(CHI_Maps(i,j,t+1)<K_th_denoise)
                        img_result_denose(i,j)=img_result_denose(i,j)+imgs(i,j,t+1);
                        cnt_result_denose(i,j)=cnt_result_denose(i,j)+1;
                    end
                    if(CHI_Maps_before(i,j,t+1)<K_th_denoise)
                        img_result_denose(i,j)=img_result_denose(i,j)+imgs_before(i,j,t+1);
                        cnt_result_denose(i,j)=cnt_result_denose(i,j)+1;
                    end
                    if(CHI_Maps_after(i,j,t+1)<K_th_denoise)
                        img_result_denose(i,j)=img_result_denose(i,j)+imgs_after(i,j,t+1);
                        cnt_result_denose(i,j)=cnt_result_denose(i,j)+1;
                    end
                end     
            end
        end
        img_denoise=img_result_denose./(cnt_result_denose+double(cnt_result_denose==1));
        figure
