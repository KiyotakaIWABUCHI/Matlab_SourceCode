function [bitplane_MC,Estimation_x,Estimation_y]=Function_ME_MDMap_CentorTime(bitplanes,Range_x,Range_y,Range_scale,Range_rotation,O_obj_Array,LabeledMDMap,Select_Area,down_sample_rate,n)
...
MDMap_3D=repmat(MDMap,1,1,number_frame);
%%%%%%%%%%%%%%%%%%%%% initialize Ed %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for ishift=Range_y(1):Range_y(3):Range_y(2)
        for m_tmp=Range_scale(1):Range_scale(3):Range_scale(2)
            ...
            for theta_tmp=Range_rotation(1):Range_rotation(3):Range_rotation(2) %-180:2:180
                ...
                weight_map_Enable=double(MDMap.*Enable_Area);
                ...
                if(cost_min>Total_cost)
                    bitplane_MC=MDMap_3D.*tmp_bitplane+(1-MDMap_3D).*bitplanes;
                    cost_min=Total_cost;
                    Estimation_x(:,:)=jshift;
                    Estimation_y(:,:)=ishift;
                end
